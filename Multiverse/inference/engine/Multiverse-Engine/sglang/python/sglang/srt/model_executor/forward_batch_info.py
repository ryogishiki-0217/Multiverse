    ...
    def init_new(
        ...
        if ret.forward_mode.is_decode():
            if ret.positions is None:
                ...
                ret.positions = clamp_position(torch.tensor(batch.right_most_pos)).to(device)
            # logger.info(f"ret.positions: {ret.positions}")
            # logger.info(f"batch.right_most_pos: {batch.right_most_pos}")
        else:
            ...
            if ret.positions is None:
                ...
                pos = []
                for i in range(len(batch.right_most_pos)):
                    try:
                        pos.append(torch.arange(batch.right_most_pos[i], batch.right_most_pos[i] + batch.extend_seq_lens[i]))
                    except Exception as e:
                        logger.error(f"Error in compute_position_triton: {e}")
                        logger.error(f"batch.right_most_pos: {batch.right_most_pos}")
                        logger.error(f"batch.extend_seq_lens: {batch.extend_seq_lens}")
                        
                pos = torch.cat(pos, dim=0)
                ret.positions = pos.to(device)
            # logger.info(f"ret.positions: {ret.positions}")
        ...
        #     logger.info(f"ret mode: {ret.forward_mode}, ret extend_num_tokens: {ret.extend_num_tokens}, ret extend_seq_lens: {ret.extend_seq_lens}, ret extend_prefix_lens: {ret.extend_prefix_lens}, ret positions: {ret.positions}")
        # else:
        #     logger.info(f"ret mode: {ret.forward_mode}, ret positions: {ret.positions}")
        return ret
