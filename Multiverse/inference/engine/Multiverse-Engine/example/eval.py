...
Replace ... with the actual answer. Ensure this exact phrase is included at the end of your response."""
            ...
            print(f"===============================")
            print(f"文件 {os.path.basename(file_path)} 处理失败: {error_msg}")
            print(f"将跳过该文件继续处理...")
            print(f"===============================\n")
            
            # 记录错误信息到结果文件
            txt_filename = os.path.splitext(os.path.basename(file_path))[0] + "_error.txt"
            txt_path = os.path.join(args.results_dir, txt_filename)
            with open(txt_path, "w", encoding="utf-8") as f:
                f.write(f"文件处理失败\n")
        ...
        if filename.endswith(".json"):  # 只处理JSON文件
            ...
            if os.path.isfile(file_path):
                ...
                if result["is_correct"]:
                    correct_count += 1
                if result["error"] is not None:  # 统计错误数
                    error_count += 1
    ...
    stats = {
        "total": total_count,
        "valid": valid_count,  # 有效处理的文件数
        "error": error_count,  # 错误文件数
    ...
    with open(stats_txt_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"处理错误数: {error_count}\n")
        f.write(f"有效评估数: {valid_count}\n")
        f.write(f"正确数: {correct_count}\n")
        f.write(f"正确率: {accuracy * 100:.2f}%\n")
    
    print(f"评估完成！总问题数: {total_count}, 处理错误数: {error_count}, 有效评估数: {valid_count}")
    print(f"正确数: {correct_count}, 正确率: {accuracy * 100:.2f}%")
